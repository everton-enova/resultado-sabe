/* Regras puras compartilhadas pelos testes e pelo Apps Script.
   Escrito compacto de proposito: o editor do Apps Script trunca colagem longa. */
var RegistrationCore = (function () {
  ...
  function validCPF(value) {
    ...
    for (var size = 9; size <= 10; size++) {
      for (var sum = 0, i = 0; i < size; i++) sum += Number(cpf[i]) * (size + 1 - i);
      var check = 11 - sum % 11;
  ...
  function normalize(input) {
    var data = { eventoId: text(input.eventoId, 30), nome: text(input.nome, 150),
  ...
  /* Evento so e utilizavel com cotas positivas e as duas datas com fuso explicito. */
  ...
  function state(event, now) {
    ...
    return now >= Date.parse(event.encerramento) ? 'ENCERRADO' : 'ABERTO';
  }
  function functionsFor(event, functions) {
    ...
    selected.forEach(function (f) {
      ...
      // Cotas compartilhadas precisam do mesmo limite, senao a contagem fica ambigua.
  ...
  function prepare(input, config, rows, now) {
    ...
    // Mesmo requestId: devolve a inscricao ja gravada, para reenvio nao duplicar linha.
    var existing = rows.find(function (r) { return r.requestId === data.requestId; });
    if (existing && existing.canonical !== canonical) fail('REQUEST_CONFLICT', 'Este envio foi alterado. Atualize a página e tente novamente.');
    ...
    var role = functionsFor(event, config.funcoes).find(function (f) { return f.id === data.funcaoId; });
    ...
    var active = activeRows(event.id, rows), group = role.grupo || role.id;
    ...
    return { record: Object.assign({}, data, { eventoNome: event.nome, funcao: nomeCompleto(role),
